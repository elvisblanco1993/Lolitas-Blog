        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow overflow-hidden sm:rounded-lg p-6">
                ...
                <form action="{{ route('post.save') }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="w-2/3">
                        <input class="w-full px-4 py-2 font-bold text-2xl rounded-lg shadow focus:outline-none focus:shadow-outline text-gray-600 @error('title') border border-red-600 @enderror" type="text" placeholder="Post Title" name="title" value="{{old('title')}}">
                        @error('title')
                    ...
                    <div class="w-full my-4">
                        <textarea class="mt-4 p-4 w-full rounded-lg text-gray-700 focus:bg-gray-100 placeholder-gray-700 @error('body') border border-red-600 @enderror" name="body" rows="20" placeholder="Write something amazing...">{{old('body')}}</textarea>
                        @error('body')
                            <small class="text-red-600">{{ $message }}</small>
                        @enderror
                    ...
                    <select class="w-1/3 px-4 py-2 shadow rounded-lg hover:bg-gray-100 hover:cursor-pointer" name="tag[]" multiple>
                    ...
                    <div class="relative mb-4 hover:cursor-pointer">
                        <input
                            type="file"
                            name="featured_image"
                    ...
                    <div class="space-y-2 my-12">
                        <label class="flex items-center space-x-4">
                          <input type="checkbox" name="comments" value="1" class="h-3 w-3 border border-gray-400 rounded-full checked:bg-gray-900 checked:border-transparent focus:outline-none">
